...
<body>
    <div class="container p-0">
        ...
        @endif
        <div
            class="vcard-one main-content  w-100 mx-auto content-blur allSection @if (getLanguage($vcard->default_language) == 'Arabic') rtl @endif">
            {{--        banner --}}
            <div class="vcard-one__banner w-100 position-relative">
                ...
                @if (strpos($vcard->cover_url, '.mp4') !== false ||
                    ...
                    <video class="cover-video {{ $coverClass }}" loop autoplay muted playsinline alt="background video" id="cover-video">
                        <source src="{{ $vcard->cover_url }}" type="video/mp4">
                    </video>
                @else
                    <img src="{{ $vcard->cover_url }}" class="img-fluid {{ $coverClass }}" alt="background image" loading="lazy" />
                @endif
                                ...
                                <div class="col-sm-6 col-12">
                                    <div class="card event-card p-3 h-100 shadow-none border-0">
                                        ...
                                        <a href="tel:+{{ $vcard->region_code }}{{ $vcard->phone }}"
                                            class="event-name text-center pt-3 mb-0 text-decoration-none" dir="ltr">+
                                            {{ $vcard->region_code }}
                                            {{ $vcard->phone }}</a>
                                    </div>
                                </div>
                                ...
                                <div class="col-sm-6 col-12">
                                    <div class="card event-card p-3 h-100 shadow-none border-0">
                                        ...
                                        <a href="tel:+{{ $vcard->alternative_region_code }} {{ $vcard->alternative_phone }}"
                                            class="event-name text-center pt-3 mb-0 text-decoration-none" dir="ltr">+{{ $vcard->alternative_region_code }}
            ...
            @if(!empty($vcard->emergencyInfo))
    <div class="vcard-one__emergency-info py-3 px-3">
        <h4 class="vcard-one-heading text-center pb-4">🆘 Datos de Emergencia</h4>
        <div class="container">
            <div class="card p-3 border-0 bg-light">
                <p><strong>🩸 Tipo de sangre:</strong> {{ $vcard->emergencyInfo->blood_type }}</p>
                <p><strong>📞 Contacto de emergencia:</strong> {{ $vcard->emergencyInfo->primary_contact_name }} ({{ $vcard->emergencyInfo->primary_contact_number }})</p>
                <p><strong>💊 Condiciones médicas:</strong> {{ $vcard->emergencyInfo->medical_conditions }}</p>

                @if(!empty($vcard->emergencyInfo->emergency_contacts['ambulancia']))
                    <p><strong>🚑 Ambulancia:</strong> {{ $vcard->emergencyInfo->emergency_contacts['ambulancia'] }}</p>
                @endif

                @if(!empty($vcard->emergencyInfo->emergency_contacts['bomberos']))
                    <p><strong>🔥 Bomberos:</strong> {{ $vcard->emergencyInfo->emergency_contacts['bomberos'] }}</p>
                @endif
            </div>
        </div>
    </div>
                                            ...
                                            <div class="card product-card p-2 border-0 w-100 h-100">
                                                ...
                                                <div class="product-details mt-3">
                                                    ...
                                                    @if ($product->currency_id && $product->price)
                                                        <span
                                                            class="text-black product-price-{{ $product->id }}">{{ $product->currency->currency_icon }}{{ getSuperAdminSettingValue('hide_decimal_values') == 1 ? number_format($product->price, 0) : number_format($product->price, 2) }}</span>
                        ...
                        <div class="container">
                            <div class="row g-3 mb-0 justify-content-center blog-slider overflow-hidden">
                                @foreach ($vcard->blogs as $blog)
                                    <div class="col-6 mb-2">
                                        <div class="card blog-card p-sm-2 p-4 border-0 w-100 h-100 flex-sm-row flex-column d-flex align-items-center"
